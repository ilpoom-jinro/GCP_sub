...
실행 작업은 먼저 AWS RDS subscription과 Cloud SQL publication을 제거해 이전
Cloud SQL -> AWS native logical replication을 정리한다. 이어서 이전 DMS job과
destination connection profile만 `--force` 없이 삭제한다. Cloud SQL 인스턴스와
private IP는 삭제하지 않는다. 그 다음 기존

...
4. 두 Deployment의 rollout 완료 여부를 확인하고 Gateway Service 주소를 출력한다.

## 주의사항

- 이 단계의 Gateway는 HTTP LoadBalancer로 GCP 직접 접속을 먼저 검증하기 위한 구성이다. Route 53 failover에 연결하기 전 HTTPS 인증서와 고정 외부 IP를 다음 단계에서 추가한다.
- Cloud SQL이 DMS standby인 동안에는 GCP를 읽기 검증 용도로만 사용한다. 쓰기 트래픽은 Cloud SQL promote 완료 전까지 허용하면 안 된다.
